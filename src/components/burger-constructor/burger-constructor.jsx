...
const BurgerConstructor = ({ ingredients }) => {
    ...
    const ingredientsCount = useMemo(() => ingredients.length - 1, [ingredients]);
                ...
                {ingredients.map((ingredient,index) => {
                    return (                        
                        <div key = {index} className = {styles.order_item + ' mr-2'}>
                            {(index > 0 && index < ingredientsCount) ? <DragIcon type="primary" /> : <span className='ml-6'></span>}
                            <ConstructorElement 
                                text = {ingredient.name} 
                                price = {ingredient.price} 
                                thumbnail = {ingredient.image} 
                                key = {index} 
                                type = {(index === 0) ? 'top' :(index === ingredientsCount) ? 'bottom' : ''}  
                            />
                        </div>
                    );
                })}
